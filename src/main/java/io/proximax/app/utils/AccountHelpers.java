...
public class AccountHelpers {
    ...
    public static void addAccountDB(LocalAccount account) throws Exception {
        if (!isExistAccount(account.fullName, account.network)) {
            ...
            String password = CryptoUtils.encryptToBase64String(account.password.getBytes(), account.password.toCharArray());
            ...
            for (String folder : CONST.DEFAULT_FOLDERS) {
                DBHelpers.addFolder(account.fullName, account.network, folder);
            }
        }
    }
    ...
    public static LocalAccount loginDB(String fullName, String network, String password) throws Exception {
        LocalAccount account = DBHelpers.getUser(fullName, network);
        if (account != null) {
            String enc_password = CryptoUtils.encryptToBase64String(password.getBytes(), password.toCharArray());
    ...
    public static LocalAccount createAccountDB(String fullName, String network, String password) throws Exception {
        Account newAccount = Account.generateNewAccount(NetworkUtils.getNetworkType(network));
        if (newAccount != null) {
            AccountHelpers.addAccountDB(new LocalAccount(fullName, password, network, newAccount.getPrivateKey(), newAccount.getPublicKey(), newAccount.getAddress().plain(), CONST.DB_VERSION));
    ...
    public static LocalAccount createAccount(String fullName, String network, String password, String privateKey) throws Exception {
        ...
        if (newAccount != null) {
            AccountHelpers.addAccountDB(new LocalAccount(fullName, password, network, newAccount.getPrivateKey(), newAccount.getPublicKey(), newAccount.getAddress().plain(), CONST.DB_VERSION));
            return login(fullName, network, password);
        }
        return null;
    }
